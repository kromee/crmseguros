...
const RESULT_BADGE: Record<string, { label: string; cls: string }> = {
  ...
  SIN_RESPUESTA: { label: "Sin respuesta", cls: "bg-slate-100 text-slate-500" },
...
export default async function DashboardPage() {
  ...
  const [dashboardData, contacts, users] = await Promise.all([
    dashboardService.getAll(session.user.id),
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex gap-2">
          ...
          <Link
            href="/pipeline/new"
            className="flex items-center gap-1.5 px-3 py-2 text-xs font-medium rounded-lg border border-slate-200 text-slate-700 hover:border-blue-300 transition-colors"
          >
      ...
      <div className="crm-card p-5">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-semibold text-slate-800">Resumen de cartera</h2>
          </div>
          ...
          <div className="crm-card p-5">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-semibold text-slate-800">Pipeline de ventas</h2>
              ...
              {pipeline.stages.map((stage, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    {!isLast && (
                      <ChevronRight className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-1/2 w-3.5 h-3.5 text-slate-300 z-10" />
                    )}
                  </div>
                );
              })}
          ...
          <div className="crm-card">
            <div className="flex items-center justify-between p-5 border-b border-slate-100">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-slate-400">Interacciones con contactos y prospectos</span>
              </div>
            ...
            {activity.length === 0 ? (
              ...
              <div className="divide-y divide-slate-50">
                {activity.map((row) => {
                  ...
                  return (
                    <div key={row.id} className="flex items-start gap-3 px-5 py-3 hover:bg-slate-50/50 transition-colors">
                      <div className="mt-0.5">
                        <span
                          className={`inline-flex items-center justify-center w-7 h-7 rounded-lg text-[10px] font-bold ${
                            ACTIVITY_BADGE[row.type] ?? "bg-slate-100 text-slate-600"
                          }`}
                        >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          <span
                            className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${
                              ACTIVITY_BADGE[row.type] ?? "bg-slate-100 text-slate-700"
                            }`}
                          >
                        ...
                        <p className="text-xs text-slate-500 mt-0.5 truncate">{row.summary}</p>
                      </div>
                      <div className="text-right flex-shrink-0">
                        <p className="text-[10px] text-slate-400" title={formatDateTime(row.createdAt)}>
                          {timeAgo(row.createdAt)}
                        </p>
                        <p className="text-[10px] text-slate-300 mt-0.5">{row.performerName}</p>
                      </div>
                    </div>
                  );
                })}
          ...
          <div className="crm-card p-5">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-semibold text-slate-800">Mi agenda hoy</h2>
                {agenda.length > 0 && (
            ...
            {agenda.length === 0 ? (
              <div className="text-sm text-slate-400 text-center py-6 border border-dashed border-slate-200 rounded-lg mb-3">
                No tienes eventos programados hoy.
              </div>
            ) : (
              <div className="space-y-1">
                {agenda.map((item, i) => {
                  const cfg = EVENT_ICON[item.type] ?? { Icon: FileText, color: "text-slate-500" };
                  ...
                  return (
                    <div key={item.id} className="flex gap-3">
                      <div className="flex flex-col items-center pt-1">
                        <div className={`p-1.5 rounded-lg ${isCompleted ? "bg-emerald-50" : "bg-slate-50"}`}>
                          {isCompleted ? (
                      ...
                      <div className={`pb-3 flex-1 ${isCompleted ? "opacity-50" : ""}`}>
                        <p className="text-xs text-slate-400">{formatTime(item.startTime)}</p>
                        <p className={`text-sm font-medium text-slate-800 mt-0.5 ${isCompleted ? "line-through" : ""}`}>
                          {item.title}
                        </p>
                        {item.contactName && (
                          <Link
...
function MetricBox({
  ...
  return (
    <div className="text-center p-3 rounded-lg bg-slate-50">
      ...
      <p className="text-[10px] text-slate-500 mt-0.5 uppercase tracking-wide font-medium">{label}</p>
    </div>
  );
}
...
function QuickLink({
  ...
  return (
    <Link
      href={href}
      className="flex items-center gap-3 px-3 py-2.5 rounded-lg border border-slate-100 hover:border-blue-200 hover:bg-blue-50/30 transition-colors group"
    >
      <Icon className="w-4 h-4 text-slate-400 group-hover:text-blue-600 transition-colors" />
      <span className="text-sm text-slate-700 group-hover:text-blue-700 flex-1">{label}</span>
