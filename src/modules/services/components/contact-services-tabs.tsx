    ...
    <div className="space-y-5">
      ...
      <div className="flex gap-2 flex-wrap">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-sm font-medium bg-white border border-slate-200 text-blue-600 hover:border-blue-300"
        >
        ...
        return (
          <div key={p.id} className="crm-card p-5">
            ...
            {p.type === "VIDA" && (p.currency || p.term) && (
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs mt-3 pt-3 border-t border-slate-100">
            ...
            {p.beneficiaries && (
              <p className="text-xs text-slate-500 mt-3">
                <span className="font-semibold text-slate-600">Beneficiarios:</span>{" "}
                {p.beneficiaries}
              </p>
            )}
    ...
    <div className="space-y-3">
      {pensions.map((p) => (
        <div key={p.id} className="crm-card p-5">
          ...
          {p.description && (
            <p className="text-xs text-slate-600 mt-3">{p.description}</p>
          )}
          {p.bitacora && (
            <div className="mt-3 p-3 rounded-lg bg-slate-50 text-xs text-slate-600 whitespace-pre-wrap">
              <span className="block text-[10px] font-semibold text-slate-400 mb-1 uppercase tracking-wide">
                Bitácora
              </span>
              {p.bitacora}
            </div>
    ...
    <div className="space-y-3">
      {vehicles.map((v) => (
        <div key={v.id} className="crm-card p-5">
          ...
          {v.description && (
            <p className="text-xs text-slate-600 mt-3">{v.description}</p>
          )}
        </div>
      ))}
    </div>
